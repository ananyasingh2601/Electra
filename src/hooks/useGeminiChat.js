...
export function useGeminiChat() {
  ...
  const sendMessage = useCallback(async (userMessage) => {
    ...
    if (!service) {
      // Demo mode — provide a helpful fallback
      const demoResponses = {
    ...
    try {
      ...
      addChatMessage({
        id: Date.now() + 1,
        role: 'assistant',
        text: "I'm sorry, I encountered an error processing your question. Please try again or check the timeline for information.",
        suggestions: ['How do I register to vote?', 'When is the next election?', 'What is the MCC?'],
        timestamp: new Date().toISOString()
      });
    } finally {
